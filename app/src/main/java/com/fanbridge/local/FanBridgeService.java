...
public class FanBridgeService extends Service {
    ...
    // Validado físicamente: velocidad 3
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        startForeground(NOTIFICATION_ID, buildNotification("Puente activo · listo para recibir órdenes"));
    ...
    private void transmit(int[] uuids, String doneText) {
        ...
        final AdvertiseCallback callback = new AdvertiseCallback() {
            ...
            public void onStartSuccess(AdvertiseSettings settingsInEffect) {
                activeCallback = this;
                updateNotification("Enviando comando…");
                handler.postDelayed(() -> {
                    if (activeCallback == this) {
                        stopCurrentAdvert();
                        updateNotification(doneText + " ✓");
                    }
                }, ADVERTISE_MS);
            }
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            channel.setDescription("Mantiene activo el puente Bluetooth del ventilador");
